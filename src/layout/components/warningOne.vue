<template>
  <el-card class="box-card warningOne">
...
export default {
  ...
  computed: {
    dataList() {
      return this.$store.state.dashord.dataListOne
    }
  },
  activated() {
  },
  ...
  methods: {
    startIntervals() {
      ...
      _this.timer = window.setInterval(function() {
        ...
        if (_this.count === 0) {
          ...
          _this.$store.commit('dashord/setWarnOnes', false)
        } else {
    ...
    endK() {
      clearInterval(this.timer)
      this.$store.commit('dashord/setWarnOnes', false)
    }
  }
}
</script>
