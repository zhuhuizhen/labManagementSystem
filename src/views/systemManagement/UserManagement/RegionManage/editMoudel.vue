<template>
  <div class="UserRegion-edit">
    <p class="bkb" />
    <el-tabs v-model="activeName">
      <!--新建数据域基本信息 :disabled="disabled"-->
      <el-tab-pane label="基本信息" name="first" class="Basic-add">
        <div>
          <el-form ref="editForm" :model="row" label-width="90px" :rules="editFormRules" class="adduser">
            <el-row :gutter="10">
              <el-col :span="12">
                <el-form-item label="编号:" prop="code" :disabled="disabled">
                  <el-input v-model="row.code" placeholder="请输入编号" :disabled="disabled" />
                </el-form-item>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="描述:" prop="description">
                  <el-input v-model="row.description" type="textarea" placeholder="请输入备注" style="width:515px;margin-left:0px;margin-top:0px;height:70px" />
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
          <div class="dialog-footer">
            <el-button size="small" class="button-sub" @click.native="submitManageForm('editForm')">下一步</el-button>
          </div>
        </div>
      </el-tab-pane>
      <!--组织选择-->
      <el-tab-pane label="组织选择" name="second">
        <div>
          <el-row>
            <!--树结构-->
            <el-col :span="8">
...
<script>
import { editRegion, getRegionOrgTree, getUserByRegionId } from '@/api/userManagement.js'
export default {
  props: {
    row: {
      type: Object,
  ...
  methods: {
    ...
    checkedOk(aaa, m) {
      // console.log(m)
      aaa.map(value => {
        if (value.checked === 'true' && (value.children === '' || value.children === null || value.children.length === 0)) {
          m.push(value.id)
        }
        if (value.children.length > 0) {
          this.checkedOk(value.children, m)
        }
      })
    },
    getRegintree() {
      const paratree = {
        regionId: this.row.id
    ...
    submitManageForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const param = { flag: '0', sysRegion: { name: this.row.name, code: this.row.code, description: this.row.description, id: this.row.id }}
          editRegion(param).then(response => {
            if (response.success === true) {
    ...
    // 组织下一步 数据域组织树选择关系新增  根据数据域ID查询用户--用户选择  2
    ...
    getRegionList() {
      ...
      getUserByRegionId(paramUser).then(response => {
        if (response.success === true) {
          this.tableDataUser = response.data
        } else {
          this.$message.error(response.msg)
        }
      }).catch(response => {
    ...
    submitUserForm() {
      ...
      // 数据域组织树新增
        ...
        editRegion({ flag: '1', sysRegion: { id: this.row.id }, sysRegionOrganizations: data }).then(response => {
          // console.log('value1', this.treeAllData)
          if (response.success === true) {
            // console.log('value2', this.treeAllData)
    ...
    handleNodeClick(dataTree) {
      // console.log('tagorgtype', dataTree.orgType)
      // console.log('dataTree', dataTree.biaoji)
    },
    checkClick(dataTree) {
      // console.log('treebiaobi', dataTree.biaoji)
      ...
      const asyncData = {}
      asyncData.regionId = this.row.id
    ...
    submitForm() {
      ...
      editRegion({ flag: '2', sysRegion: { id: this.row.id }, userIdList: ids }).then(res => {
        if (res.success === true) {
          this.$message.success('保存成功')
        } else {
