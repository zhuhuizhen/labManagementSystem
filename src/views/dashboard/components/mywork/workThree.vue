<template>
  <div class="workContent">
    <ul :style="{transform:transform}" :class="{rollScreen_list_unanim:num===0}" @mouseover="stopTime" @mouseleave="continueTime">
      <li v-for="(item,index) in eventsThree" :key="index" @click="goTraffic(item.id)">
      ...
      <li v-for="(item,index) in eventsThree" v-show="eventsThree.length > 5" :key="index+eventsThree.length" @click="goTraffic(item.id)">
        <span class="workMile" :title="item.mileage">{{ item.mileage }}</span>
        <span v-if="item.content" class="workName">{{ item.content }}</span>
        <span v-else class="workName" v-html="type(item.type)" />
        <span class="workB">
...
export default {
  ...
  watch: {
    numLengthThree(val, oldVal) {
      // 轮播
      // debugger
      this.timeLength = this.eventsThree.length
      const _this = this
      if (val === -1) {
      ...
      if (_this.numLengthThree <= 5) {
        return
      } else {
        _this.tiomeOne = window.setInterval(function() {
          if (_this.num !== _this.eventsThree.length) {
            _this.num++
          // setTimeout(function () {
          //   _this.num++
          // }, 50)
  ...
  methods: {
    ...
    stopTime() {
      ...
      this.$emit('stopLengthThree', -1)
    },
    continueTime() {
      this.$emit('stopLengthThree', this.timeLength)
    },
    goTraffic(id) {
      const querys = {
        id: id
      }
      this.$router.push({ path: '/projectManage/trafficDetail/' + id, query: querys })
    }
  }
}
</script>
