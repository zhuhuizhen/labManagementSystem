<template>
  <div class="workContent">
    <ul :style="{transform:transform}" :class="{rollScreen_list_unanim:num===0}" @mouseover="stopTime" @mouseleave="continueTime">
      <li v-for="(item,index) in events" :key="index" @click="goConstruction(item.id)">
      ...
      <li v-for="(item,index) in events" v-show="events.length > 5" :key="index+events.length" @click="goConstruction(item.id)">
        <span class="workMile" :title="item.mileage">{{ item.mileage }}</span>
        <span v-if="item.content" class="workName">{{ item.content }}</span>
        <span v-else class="workName" v-html="type(item.type)" />
        <span class="workB">
...
export default {
  ...
  watch: {
    numLength(val, oldVal) {
      // 轮播
      // debugger
      this.timeLength = this.events.length
      const _this = this
      if (val === -1) {
      ...
      if (_this.numLength <= 5) {
        return
      } else {
        _this.tiomeOne = window.setInterval(function() {
          if (_this.num !== _this.events.length) {
            _this.num++
          // setTimeout(function () {
          //   _this.num++
          // }, 50)
  ...
  methods: {
    ...
    goConstruction(id) {
      ...
      this.$router.push({ path: '/projectManage/constructionDetail/' + id, query: querys })
    }
  }
}
</script>
